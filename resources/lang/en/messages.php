<?php
return [
    'INF_INS_SUCCESS' => '登録しました。',
    'INF_UPD_SUCCESS' => '更新しました。',
    'INF_DEL_SUCCESS' => '削除しました。',
    'INF_UN_SHIP_SUCCESS' => 'This record has been un ship successfully.',
    'WRN_NOT_FOUND' => '検索結果がありません。',
    'WRN_NOT_SELECT_CHKBOX' => 'no selected checkBox',
    'ERR_MSG_DUPLICATE' => 'CANNOT INSERT A DUPLICATE KEY!',
    'ERR_NO_SET_EX_RATE' => 'Please registar Exchange Rate. (USD => %s)',
    'INF_NOT_CHECK' => '請求書のメール送信先を選択してください。',
    'INF_SEND_CLAIMMAIL' => '請求対象の会社へメールを送信しました。',
    'INF_NOT_SEND_CLAIMMAIL' => '請求対象外のため、メール送信を中止しました。',
];
